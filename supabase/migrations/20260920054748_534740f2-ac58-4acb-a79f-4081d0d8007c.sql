...
BEGIN
  ...
  ) AS candidates(user_id, plan_date, subject_id, subject_name, chapter_id, chapter_name,
    session_kind, target_minutes, priority, source, pinned, review_stage, next_review_at,
    status, rank_score, class_id)
  WHERE NOT EXISTS (
    ...
    WHERE e.user_id = p_user_id AND e.plan_date = p_plan_date AND e.status <> 'cancelled'
      AND COALESCE(e.subject_id::text, '') = COALESCE(candidates.subject_id::text, '')
